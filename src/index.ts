...
// Start unconditionally. Upstream guarded this with `require.main === module`,
// but Claude Desktop launches the server via Electron's UtilityProcess, where
// `require.main !== module` — so the guard silently skipped main(), the server
// never started, and the MCP handshake timed out ("Unable to connect"). This
// file is only ever run as a process entry point (startServer is exported for
// tests but never imported at runtime), so an unconditional call is safe.
main().catch((err) => {
